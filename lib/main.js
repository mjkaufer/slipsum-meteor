...
UI.registerHelper('slipsum', function(paragraphs, pTag, headerTag, onlyHeader){//paragraphs = amount of paragraphs, pTag is the tag in front of p, headerTag is an optional value to fill out - if left empty, there will be no headers. If set as "", headers will start with "" (nothing). If set as "<h1>", it'll start with <h1> and end with </h1>. Etc.
  
  var ret = "";
  var start = end = extra = "";

  var shuffleIpsum = shuffle(ipsum.slice(0));/*copies it then shuffles*/
  var shuffleSlips = shuffle(slips.slice(0));/*copies it then shuffles*/

  if(typeof paragraphs === 'undefined')
    paragraphs = 1;

  if(onlyHeader !== true)//undefined or otherwise
    onlyHeader = false;//so paragraphs are displayed by default

  if(typeof includePTag === 'undefined'){//true by default
    start = "<p>";
    end = "</p>";
  }
  ...
  for(var i = 0; i < paragraphs; i++){
      if(headerTag !== undefined){
        ...
        ret += headerTag.replace("<","</");//now it's an end tag
      }
      if(!onlyHeader){
